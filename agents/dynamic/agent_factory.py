"""
Упрощенная фабрика для создания динамических агентов из БД.
Принципы: простота, кэширование, минимум БД запросов.
"""
import json
...
class DynamicAgentFactory:
    """
    Упрощенная фабрика для создания динамических агентов.
    Быстрые операции с кэшированием конфигураций.
    """
    ...
    def create_agent_from_db(
        ...
        """
        Создает агента из БД с кэшированием конфигурации.
        """
        try:
            ...
            # Добавляем компоненты если включены
            if memory_config.get('enabled', False):
                memory = DynamicAgentFactory._create_memory(model_id, memory_config)
                if memory:
                    agent_params['memory'] = memory
                    agent_params['enable_agentic_memory'] = True
            
            if storage_config.get('enabled', True):
                storage = DynamicAgentFactory._create_storage(storage_config)
                if storage:
            ...
            if knowledge_config.get('enabled', False):
                knowledge = DynamicAgentFactory._create_knowledge(knowledge_config)
                if knowledge:
            ...
            for key, value in settings.items():
                if key in ['debug_mode', 'markdown', 'add_datetime_to_instructions', 
                          'show_tool_calls', 'reasoning', 'retries']:
    ...
    @staticmethod
    def _create_tools(tools_config: List[Dict[str, Any]]) -> List[Union[Function, Toolkit]]:
        ...
        for tool_config in tools_config[:10]:  # Ограничиваем 10 инструментами
            try:
                ...
                if tool_type == 'static':
                    tool = DynamicAgentFactory._import_static_tool(
    ...
    @staticmethod
    def _import_static_tool(import_path: str, init_params: Dict[str, Any]) -> Optional[Union[Function, Toolkit]]:
        """Импорт статического инструмента"""
        try:
            if not import_path:
                return None
    ...
    @staticmethod
    def _create_memory(model_id: str, memory_config: Dict[str, Any]) -> Optional[Memory]:
        """Создание памяти"""
        try:
    ...
    @staticmethod
    def _create_storage(storage_config: Dict[str, Any]) -> Optional[PostgresAgentStorage]:
        """Создание хранилища"""
        try:
    ...
    @staticmethod
    def _create_knowledge(knowledge_config: Dict[str, Any]) -> Optional[AgentKnowledge]:
    ...
    def get_dynamic_agent_ids() -> List[str]:
        """Получение списка ID динамических агентов"""
        try:
            with SessionLocal() as session:
                query = text("""
                    SELECT agent_id 
